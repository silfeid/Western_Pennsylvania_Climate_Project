...
def func_quit():
    ...
    if os.path.exists('C:/Users/brode/Python/Western_PA_Climate_Project/Raw_CSVs'):
        rmtree('C:/Users/brode/Python/Western_PA_Climate_Project/Raw_CSVs')
    sys.exit()
...
def slice_dfs():
    ...
    for key in df_dict.keys():
        ...
        if max(df_dict[key].index) < end:
            ...
            df_dict[key] = df_dict[key].loc[start:trunc_end]
        else:
...
def single_df_plotter():
    ...
    fig.savefig('Western_PA_Climate_Project/Plots/Single_Stations/'+df_text+'/'+df_text+'_'+print_variable+'_'+start+'-'+end+'.jpg')
...
def descriptive_stats_grapher():
    ...
    #Now we repeat the same process over again, but this time using our refine_dfs_for_snow() function to remove non-winter months:
    """I don't like how these variable names are repeated ... makes me uneasy"""
    df_dict, start, end = refine_dfs_for_snow()
    start = str(start)
    end = str(end)
    #turn the keys of the df_dict into a list that we can iterate over; don't necessarily have to cast it as a list, but seems a bit safer...
    df_keys =list(df_dict.keys())
    ...
    for key in df_keys:
        if key in df_dict:
            ...
            try:
                snowfall_dict[key] = float(df_dict[key].loc['snow_fall'])   
            except KeyError:
                pass
            try:
    ...
    valid_av_var_choices = {1:snowfall_dict, 2:snowdepth_dict, 3:tempmin_dict, 4:tempmax_dict}
    
    if av_var_choice in valid_av_var_choices.keys():
        
        av_var_chosen = valid_av_var_choices[av_var_choice]
        if av_var_choice == 1:
        ...
        plt.savefig('Western_PA_Climate_Project/Plots/All_Stations/Means_Comparisons/'+display_var+'_All_Stations-'+start+'-'+end+'.jpg')
...
def comparison_plotter():
    ...
    #And create this variable, a tally of the total number of observations graphed, to be displayed on the x-axis.  
...
def menu():
    ...
    if menu_choice in menu_choices:
        ...
        if menu_choice == '6':
            ...
            print('\nStation Count: '+str(len(station_dict)))    
            menu()
        if menu_choice == '7':
            show_slicer_dates()
            menu()
