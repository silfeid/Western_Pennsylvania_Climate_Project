...
#connstring is saved to file, because we're going to need to change it once we create the db; we need to start off with a dbname that we know will exist (postgres); once we've created the western_pa_climate_db, we will write that db name to file and read it back into Python as the connection string. I think that this must be a sort of clumsy way of doing this, but it has worked just fine without giving me any guff the whole time that I've been working on the project, so I'm going to leave it be for now.
